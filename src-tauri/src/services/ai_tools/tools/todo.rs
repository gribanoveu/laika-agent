...
fn write(todos: &[Task], titles: &[String]) -> Result<Vec<Task>, ToolError> {
    if todos.len() + titles.len() > MAX_TASKS {
    ...
    for (offset, title) in titles.iter().enumerate() {
        updated.push(Task {
            // Sequential over the whole list, which is append-only, so an id
            // is never reused even after tasks are completed.
            id: format!("t{}", todos.len() + offset + 1),
            title: title.clone(),
