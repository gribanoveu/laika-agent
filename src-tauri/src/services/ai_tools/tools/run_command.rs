...
pub(super) fn definition() -> LlmToolDefinition {
    LlmToolDefinition {
        ...
        description: format!("Run a shell command in the workspace — build, test, lint, inspect. This is how you check your own work: after changing code, run the tests rather than claiming they pass. It runs under `/bin/sh -c` (`cmd.exe /C` on Windows), not the user's shell: write POSIX sh, not bash or zsh. The exit code, stdout and stderr all come back; a non-zero exit is an ordinary answer, not a failure of the call. The exit code is the whole line's — in `a; b` it is `b`'s, so use `&&` when an earlier failure matters. Output is streamed as it is produced; stdout and stderr are cut separately: past {MAX_OUTPUT_CHARS} characters, a stream keeps its first and its last {half} characters and drops the middle, and the mark there says how many lines (or characters, when a long line was cut) went — for a long build or test log, redirect it to a file (`> build.log 2>&1`) and grep that for what matters. The command runs to completion or is killed at its timeout, together with everything it started. For something that has to keep running — a dev server, a watcher — set background: the call returns at once with a process number, readOutput reads what it writes, stopProcess ends it, and you are told when one ends on its own.", half = MAX_OUTPUT_CHARS / 2),
