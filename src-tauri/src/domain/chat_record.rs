...
pub enum ChatError {
    ...
    AppDir(String),
    #[error("could not read the chat: {0}")]
    Read(std::io::Error),
...
/// Ids come back from the window, and this one becomes a file name. Anything
...
mod tests {
    ...
    /// The id becomes a file name, and it arrives from the window.
    #[test]
