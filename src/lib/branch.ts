 ...
 * a digest that also covers what came after. The first mismatch walking back
 * is where the summary starts, and everything before it is out of reach.
 */
export function branchPoints(blocks: Block[], history: LlmMessage[]): Map<string, number> {
  ...
  for (let b = bubbles.length - 1, m = asked.length - 1; b >= 0 && m >= 0; b--, m--) {
    const bubble = bubbles[b];
    if (bubble.kind !== "user" || history[asked[m]].content !== bubble.text) break;
