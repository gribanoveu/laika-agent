...
перенести в ядро**: решение «что отправить модели» обязано быть проверяемым без окна, а
1800 строк промпт-сборки во фронтенде не тестируются вместе с циклом, который они кормят.
Логику берём, слой меняем.
